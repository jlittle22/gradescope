...
class GSCourse():
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~ASSIGNMENTS~~~~~~~~~~~~~~~~~~~~~~~~~~
    def get_assignments_listing(self):
        self.assignments = {}
        self._lazy_load_assignments()

        assignments_listing = {}
        for name, assignment in self.assignments.items():
            assignments_listing[assignment.aid] = {
                "name" : name, 
                "points" : assignment.points,
                "percent_graded" : assignment.percent_graded,
                "complete" : assignment.complete,
                "regrades_on" : assignment.regrades_on
            }

        return assignments_listing
    ...
    def get_assignment(self, aid):
        ...
        for name, val in self.assignments.items():
            if aid == val.aid:
                for q in val.get_questions():
                    assignment_data["submissions"][q['qid']] = val.get_submissions_grading_status(q['qid'])                
                assignment_data["statistics"] = val.get_statistics()
